...
# model = RetinaFaceModel(cfg, training=False, iou_th=0.4, score_th=0.5,input_size=(480, 288, 3))
# checkpoint_dir = 'retinaface_mbv2'
# checkpoint = tf.train.Checkpoint(model=model)
# checkpoint.restore(tf.train.latest_checkpoint(checkpoint_dir))
# tf.saved_model.save(model,'tfmodel_org')
...
# org_model output link -> 1KuzZWZqYCIfZWwBwiG2TPFOYkDjwxSfP
# model link for number of faces as output -> 1k0Xs9lCPWfQulk0J1CAlcqW93XoSLoZR

gdd.download_file_from_google_drive(file_id='1KuzZWZqYCIfZWwBwiG2TPFOYkDjwxSfP',
                                   dest_path='./tfmodel_org.zip',
                                   unzip=True)
...
# Convert the model.
converter = tf.lite.TFLiteConverter.from_saved_model('tfmodel_org')
...
# Save the model.
with open('litemodel_org_uin8.tflite', 'wb') as f:
  f.write(tflite_quant_model)

# Test lite model
interpreter = tf.lite.Interpreter(model_path="litemodel_org_uin8.tflite")
...
valid_result = interpreter.get_tensor(output_details[1]['index'])

if valid_result>0:
    print(output_data[:int(valid_result)])
else:
    print('There is no valid face')